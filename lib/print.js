...
export const printStruk = async (items, outlet, kasir, paymentType) => {
  try {
    const date = new Date();
    ...
    // Total
    BLEPrinter.printText("<C>------------------------------</C>\n");
    BLEPrinter.printText(`<C><B>TOTAL: Rp. ${total.toLocaleString("id-ID")}</B></C>\n`);
    BLEPrinter.printText("<C>------------------------------</C>\n\n");
    ...
    BLEPrinter.printText(`<L>Metode Pembayaran: ${paymentType}</L>\n`);
    BLEPrinter.printText(`<L>Bayar: Rp. ${bayar.toLocaleString("id-ID")}</L>\n`);
